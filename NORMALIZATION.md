...
- **RFC 5321** (SMTP) defines the local part of an email address as opaque to
  the transport layer. The receiving mail server is the sole authority on
  whether two local-part strings identify the same mailbox.
- **RFC 5322** (Internet Message Format) specifies the syntax of email addresses
  but does not prescribe any equivalence rules for the local part.
- **RFC 4343** defines that domain names are case-insensitive.
- **RFC 5233** defines the _Sieve subaddress extension_ ("`+`" tagging), which
  is optional and must be enabled by the receiving server. It is not universally
  supported.
...
follows the RFC 5321 principle that the local part is opaque to external
systems.
...
**References:**
[Gmail Help – About dots in addresses](https://support.google.com/mail/answer/7436150),
...
**References:**
[Yahoo Help – Disposable email addresses](https://help.yahoo.com/kb/SLN2028.html)
...
local part is stripped server-side rather than being used as a subaddress
separator.
...
**References:**
[Apple Support – iCloud Mail: Use plus addressing](https://support.apple.com/guide/icloud/use-plus-addressing-mm6b1a0955/icloud)
(Note: Apple's `+tag` support is also described in their iCloud Mail
documentation; alias domains `me.com` and `mac.com` are legacy names for
`icloud.com`.)
...
**References:**
[Fastmail Help – Plus addressing](https://www.fastmail.help/hc/en-us/articles/1500000280261)
...
**References:**
[Proton Help – What is my Proton Mail address](https://proton.me/support/email-alias-addresses)
...
**References:**
[Yandex Help – Email address rules](https://yandex.com/support/mail/mail-clients.html)
...
Domain alias resolution maps non-canonical domains to their canonical
equivalents. **Only explicit, provider-documented mappings are used.** No DNS
lookups or heuristics are performed.
...
1. Add the provider's domains to `domains.go` as
   `var <provider>Domains = []string{...}`.
...
3. Register every known domain for the provider in `NewNormalizer()` in
   `normalizer.go`.
4. Create `<provider>_rule_test.go` covering `ProcessUsername` and
   `ProcessDomain`.
5. Document the normalization behavior in the table above and cite the
